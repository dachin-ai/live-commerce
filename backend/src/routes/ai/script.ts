...
import { runScriptResearch, buildLLMSystemPrompt, buildLLMUserMessage, buildCozeScriptPrompts, buildScriptToolCallMessage } from '../../rules/scriptResearch'
...
async function parseScriptRequestBody(body: Record<string, unknown>): Promise<{
  ...
  // 支持 full_process（Coze 风格）映射到 full-sales（项目现有类型）
  const scriptTypeStr = String(rawScriptType)
  const scriptType = scriptTypeStr === 'full_process' ? 'full-sales' : scriptTypeStr

  const hasProductName = rawProductName != null && String(rawProductName).trim() !== ''
  const explicitScriptType = ['interaction', 'scenario', 'promotion', 'closing', 'full-sales', 'full_process'].includes(
  ...
  const userInput = {
    ...
    scriptType: (explicitScriptType ? scriptType : 'interaction') as ScriptType,
...
今天直播间${priceStr}！家人们，这价你在外边真找不到——限时有效，要的抓紧！小黄车点开，直接下单！`
  }
...
就现在这价、这赠品！库存不多了，手慢无！3、2、1——下单！小黄车点开没？别犹豫了，马上点小黄车一键下单！`
  }

  if (scriptType === 'scenario') {
    return `【场景化塑品话术】

${t}的家人们，你们是不是也想——（场景痛点）？今天这款${p}，${f}，正好解决这个问题！用起来（效果描述）。咱们直播间${priceStr}，要的赶紧小黄车下单！`
  }

  if (scriptType === 'interaction') {
    return `【人群互动话术】

家人们，今天这款${p}，${f}！${priceStr}！觉得需要的扣1，有问题公屏打出来！点赞关注不迷路，小黄车在下方，需要的直接拍！`
  }
...
router.post('/stream', async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    const hasCTA = ['interaction', 'promotion', 'closing', 'full-sales'].includes(safeScriptType)
    const usePromotionInfo = ['promotion', 'closing', 'full-sales'].includes(safeScriptType)
    ...
    const script = {
      ...
      script_type: safeScriptType === 'full-sales' ? 'full_process' : safeScriptType,
