...
1. 列出 agent 的记忆文件（如有）：
   - Gemini Antigravity：检查 `~/.gemini/antigravity/brain/<conversation-id>/` 下的 artifacts
   - Claude Code：`ls ~/.claude/projects/<...>/memory/` 并读 `MEMORY.md`
   - 其他平台：找该 agent 的等价位置（见 [references/agent-paths.md](references/agent-paths.md)）
2. 对本次对话涉及的**每一个项目**：
   - **Windows**：`Get-ChildItem <project-root> -Depth 0` → 确认根目录结构
   - **Windows**：`Get-ChildItem <project-root>\docs -Filter *.md` → 枚举所有 docs
   - **Windows**：`Get-ChildItem <project-root> -Filter *.md -Recurse -Depth 2 | Where-Object { $_.FullName -notmatch 'node_modules|\.git' }` → 兜底抓散落的 .md
   - **Unix**：`find <project-root> -maxdepth 2 -name "*.md" -not -path "*/node_modules/*" -not -path "*/.git/*"`
   - 读 `README.md`、`CLAUDE.md` / `AGENTS.md`（如有）、关键 `docs/*.md`
3. 读全局 agent 配置（若有，见 [references/agent-paths.md](references/agent-paths.md)）
4. 回顾本次对话全部内容

**输出一张文件清单**（内部用，不用给用户看），对每个文件标：「评估过 / 要改 / 不用改」。**漏一个不行**——这是这个 skill 最容易翻车的地方。

#### 本项目 docs/ 分类速览（110+ 文件）

| 分类 | 关键文件 | 高频变动？ |
|------|---------|----------|
| **启动与部署** | 启动与数据库操作指南、部署到域名操作指南、部署前全局检查清单 | 中 |
| **LLM/AI 功能** | LLM交互流程说明、话术生成系列（20+）、待办生成系列（10+） | 高 |
| **Coze 对接** | Coze对接说明-待办与话术入参出参、Coze优化说明文档 | 高 |
| **数据导入** | TikTok数据导入使用指南、数据统计口径说明、数据维度扩展-重新导入指南 | 中 |
| **系统架构** | 项目结构说明、配置与特性开关说明、角色与权限矩阵 | 低 |
| **测试** | 完整功能测试指南、功能测试清单、用户验证指南 | 中 |
...
**跨项目改动**：本次对话改了多个项目，每个项目都要跑一次完整的第一步。不要假设一个项目的 docs 改了，另一个就不用。尤其是上游-下游对接文档（集成指南 / SDK 说明 / API 协议），两边都要对齐。

**发现之前的同步漏了东西**：修掉。不要说"那不是这次对话的事"——你就是这个项目的持续编辑，过去的漏洞也归你管。

## 本项目当前技术栈（2026-05-01）

> 同步文档时务必确认以下信息在各文档中保持一致：

| 组件 | 技术 | 变更日期 |
|------|------|----------|
| 数据库 | PostgreSQL 18.3（本地压缩包部署） | 2026-04-30 |
| 后端 | Express + TypeScript + pg | 持续迭代 |
| 前端 | React + TypeScript + Vite | 持续迭代 |
| AI/LLM | Coze Agent API + Gemini System Agent | 持续迭代 |
| 部署 | 本地开发 + 远程 VPS（nginx 反代） | 持续迭代 |

## 参考资料

- **[references/sync-matrix.md](references/sync-matrix.md)** — 完整的"变更类型 → 要改哪些文件"映射表（✅ 已创建）
- **[references/agent-paths.md](references/agent-paths.md)** — 各 Agent 平台的记忆与配置路径速查（✅ 已创建）
